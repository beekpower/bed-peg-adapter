...
LIP      = 1.8      # how far each snap lip reaches over the top edges of the rail
...
WALL_T   = 3.0      # side wall thickness (thinner = easier snap, less grip)
...
def side_wall(sign):
    x_in, x_out = sign * half_in, sign * half_out
    lip_h = LIP + 0.6
    wall = box(min(x_in, x_out), max(x_in, x_out), -LENGTH / 2, LENGTH / 2, 0, top + lip_h)
    # snap lip: flat underside that hooks the rail, 45 deg ramp on top so it
    # cams open when pressed down over the rail
    pts = [V(x_in, -LENGTH / 2, top),
           V(x_in - sign * LIP, -LENGTH / 2, top),
           V(x_in - sign * LIP, -LENGTH / 2, top + 0.6),
           V(x_in, -LENGTH / 2, top + lip_h),
           V(x_in, -LENGTH / 2, top)]
    wall = wall.fuse(Part.Face(Part.makePolygon(pts)).extrude(V(0, LENGTH, 0)))
    if SCREW_D:
        hole = Part.makeCylinder(SCREW_D / 2, WALL_T + 2, V(x_in - sign * 1, 0, RAIL_H / 2),
                                 V(sign, 0, 0))
        wall = wall.cut(hole)
    return wall
